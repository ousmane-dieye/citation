...
let i = 0; // index de la citation actuelle
let aleatoire; // nombre aleatoire pour trouver la prochaine citation
let max = citations.length // nombre de citation 


function genererNombreEntier(max) {
  return Math.floor(Math.random() * max); // Math.random donna aleatoirement un decimal entre 0 inclus et 1 exclus, max est le nombre de cittion et Math.floor() arrondi les decimaux a l'entier inferieur 
}

btn.addEventListener('click', () => {
    do{
        aleatoire = genererNombreEntier(max);
    }while (aleatoire == i); // tant que le nombre generer est le meme que le precedant
    i = aleatoire;
    citation.textContent = citations[i][0];
    auteur.textContent = citations[i][1];
    console.log("fin")
})